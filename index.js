...
app.listen(port, () => {
  console.log(chalk.green(`ðŸŒ Puerto ${port} esta abierto`));
});
...
async function start(file) {
  ...
  p.on('exit', (_, code) => {
    isRunning = false
    console.error('âŽ OcurriÃ³ un error inesperado:', code)
    start('main.js'); //
  ...
  console.log(chalk.yellow(`ðŸ“ƒ Script by FG98`));
  ...
  const packageJsonPath = path.join(path.dirname(currentFilePath), './package.json');
    ...
    console.error(chalk.red(`âŒ No se pudo leer el archivo package.json: ${err}`));
  }
